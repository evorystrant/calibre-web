...
def upload():
    ...
    if request.method == 'POST' and 'btn-upload' in request.files:
        ...
        for requested_file in request.files.getlist('btn-upload'):
            ...
            # return redirect(url_for("index"))
            # if current_user.role_edit() or current_user.role_admin():
            #     result += render_title_template('book_edit.html', book=db_book, authors=author_names, cc=cc, title=_(u"edit metadata"))
            #     continue
            # book_in_shelfs = []
            # result += render_title_template('detail.html', entry=db_book, cc=cc, title=db_book.title, books_shelfs=book_in_shelfs, )
        # return result
        return redirect(url_for("index"))
    else:
        return redirect(url_for("index"))
